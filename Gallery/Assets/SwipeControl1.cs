...
public class SwipeControl1 : MonoBehaviour {
    ...
    private void Redraw()
    {
        ...
        {
            images[i].transform.SetPositionAndRotation(transform.TransformVector(new Vector3(
                radius * Mathf.Sin(2 * Mathf.PI * (float)i / (float)images.Length + phi), transform.position.y, radius * Mathf.Cos(2 * Mathf.PI * (float)i / (float)images.Length + phi) - 0.1f)), Quaternion.AngleAxis(10, Vector3.forward));
    ...
    private void Start()
    {
        ...
        radius = 0.025f * (float)images.Length / 2;
        Redraw();
    }
